    ...
    ILogger<AuthService> logger) : IAuthService
{
    ...
    public async Task<RegisterResponseDto> RegisterAsync(RegisterDto registerDto)
    {
        ...
        if (await userRepository.ExistsByEmailAsync(registerDto.Email))
        {
            ...
            throw new BussinessException(ErrorCodes.EMAIL_ALREADY_EXISTS, "Email already exists");
        }
        ...
        if (await userRepository.ExistsByUsernameAsync(registerDto.Username))
        {
            ...
            throw new BussinessException(ErrorCodes.USERNAME_ALREADY_EXISTS, "Username already exists");
        }
        ...
        if (registerDto.ProfilePicture != null && registerDto.ProfilePicture.Size > 0)
        {
            ...
            if (!isValid)
            {
                ...
                throw new BussinessException(ErrorCodes.INVALID_FILE_FORMAT, errorMessage!);
            }
            ...
            catch (Exception)
            {
                ...
                throw new BussinessException(ErrorCodes.IMAGE_UPLOAD_FAILED, "Failed to upload profile image");
            }
        }
        else
        {
        ...
        logger.LogUserResgistered(createdUser.Username);
    ...
    private UserResponseDto MapToUserResponseDto(User user)
    {
        ...
        return new UserResponseDto
        {
            ...
            Role = userRole,
            Status = user.Status,
            IsEmailVerifield = user.UserEmail?.EmailVerifield ?? false,
    ...
    public async Task<EmailResponseDto> VerifyEmailAsync(VerifyEmailDto verifyEmailDto)
    {
        ...
        user.UserEmail.EmailVerifield = true;
        user.Status = true;
    ...
    public async Task<EmailResponseDto> ResendVerificationEmailAsync(ResendVerificationDto resendDto)
    {
        ...
        if (user.UserEmail.EmailVerifield)
        {
            return new EmailResponseDto
            {
                Sucess = false,
